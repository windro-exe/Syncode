...
import { PRUNE_TURN_TRIGGER_FRACTION } from "@/session/compaction"
...
const tokenize = (messages: SessionV1.WithParts[], kind: "text" | "tool") =>
  Effect.gen(function* () {
    const strings = messages.flatMap((msg) =>
      msg.parts.flatMap((part: SessionV1.Part) => {
...
function lastAssistantTokens(messages: SessionV1.WithParts[]) {
        ...
        Effect.gen(function* () {
          ...
          if (model && model.limit.context > 0) {
            ...
            if (budget > 0) {
              ...
              const pruneAt = budget * PRUNE_TURN_TRIGGER_FRACTION
