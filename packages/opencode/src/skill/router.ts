...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const route = Effect.fn("SkillRouter.route")(function* (input: RouteInput) {
      ...
      if (!userText) {
        yield* Effect.logDebug("no userText, skipping router")
        return [] as string[]
      }

      const routerModel = yield* resolveRouterModel(provider, cfg, input.fallbackModel)
      if (!routerModel) {
        yield* Effect.logWarning("no small router model available, skipping route to avoid tapping main-model quota")
        return [] as string[]
      }
      ...
      const result = yield* llm
        ...
        .pipe(
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              yield* Effect.logInfo("router failed", { cause: String(cause).slice(0, 200) })
      ...
      yield* Effect.logInfo("router decision", { picked, raw: result.slice(0, 240) })
      return picked
    })
...
const resolveRouterModel = Effect.fn("SkillRouter.resolveRouterModel")(function* (
  ...
  if (explicit) {
    ...
    if (slash > 0) {
      ...
      yield* Effect.logWarning("router_model not found, falling back to small_model", { explicit })
    } else {
      yield* Effect.logWarning("router_model is malformed (expected 'providerID/modelID'), falling back to small_model", { explicit })
    }
  }
...
export const node = LayerNode.make({
  service: Service,
  layer,
  deps: [Skill.node, LLM.node, Provider.node, Config.node],
})
