...
// Extends the AI SDK's APICallError on purpose. opencode classifies provider
// failures in provider/error.ts + session/message-v2.ts, and BOTH gate on
// `APICallError.isInstance`. A plain Error falls through to NamedError.Unknown,
// which means: a context-overflow reject never becomes a ContextOverflowError
// (so the session dead-ends instead of compacting and retrying), and 5xx/429
// are never retried. Subclassing keeps the AI SDK's Symbol.for markers, so
// isInstance works even across duplicate @ai-sdk/provider copies.
export class KiroApiError extends APICallError {
  override name = "KiroApiError"
  constructor(input: { status: number; body: string; url?: string; request?: unknown }) {
    super({
      message: `Kiro API error ${input.status}: ${input.body}`,
      url: input.url ?? "https://q.amazonaws.com/",
      requestBodyValues: input.request ?? {},
      statusCode: input.status,
      responseBody: input.body,
      // Transient AWS failures: throttling, request timeout, and every 5xx.
      isRetryable: input.status === 408 || input.status === 429 || input.status >= 500,
      data: { status: input.status, body: input.body },
    })
  }
}
...
// Last-resort window used only if a model is missing from the catalog. Any real
// model MUST resolve through contextWindow() — see the invariant test in
// test/provider/kiro-context.test.ts.
const FALLBACK_CONTEXT = 2e5

// AWS Q never reports token counts. It reports a `contextUsageEvent` carrying a
// PERCENTAGE of the window, so a percentage is all we can turn into tokens — and
// the multiplier has to be the window of the model actually in use.
//
// Read it from the same catalog the session layer compares against
// (session/overflow.ts thresholds are derived from `model.limit.context`). A
// second constant living here is what broke: it said 200_000 for every model
// while overflow.ts sized its thresholds off 640_000, so reported usage
// saturated at 200_000 (Q's 100%) — a third of the prune trigger. Nothing ever
// trimmed and Q hard-rejected with CONTENT_LENGTH_EXCEEDS_THRESHOLD instead.
export function contextWindow(modelId: string): number | undefined {
  return BUILTIN_PROVIDERS["kiro"]?.models[modelId]?.limit.context
}
...
export class KiroLanguageModel implements LanguageModelV3 {
  ...
  /** Window used to turn Q's context-usage percentage into a token count. */
  private contextWindow(): number {
    return this.config.context ?? contextWindow(this.modelId) ?? FALLBACK_CONTEXT
  }
  ...
  private async callApi(token: string, body: string): Promise<Response> {
    ...
    const response = await (this.config.fetch ?? globalThis.fetch)(endpoint, {
      method: "POST",
      headers: {
        ...authHeaders(token),
    ...
    if (!response.ok)
      throw new KiroApiError({
        ...
        url: endpoint,
        request: { modelId: this.modelId, bytes: body.length },
      })
    return response
  }
  ...
  async doStream(options: LanguageModelV3CallOptions): Promise<LanguageModelV3StreamResult> {
    ...
    return {
      stream: source.pipeThrough(makeTransform(this.contextWindow())),
      request: { body },
