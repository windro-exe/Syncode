...
function renderForExtraction(messages: SessionV1.WithParts[]): string {
  const chunks: string[] = []
  ...
  Effect.gen(function* () {
    ...
    const extract = Effect.fn("AutoMemory.extract")(function* (input: ExtractInput) {
      ...
      if (inFlight.has(input.sessionID)) {
        yield* Effect.logInfo("auto-memory: extract skipped (already in flight)", { "session.id": input.sessionID })
        return
      }
      ...
      const result = yield* llm
        ...
        .pipe(
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              yield* Effect.logInfo("auto-memory extraction failed", { cause: String(cause).slice(0, 200) })
      ...
      if (!facts || /^none\.?$/i.test(facts)) {
        ...
        for (const id of ids) seen.add(id)
        yield* Effect.logInfo("auto-memory: nothing worth keeping", { messages: targets.length })
        return
      }
      ...
      if (!newFacts.trim()) {
        for (const id of ids) seen.add(id)
        yield* Effect.logInfo("auto-memory: all facts already known", { messages: targets.length })
        return
      }
      ...
      const appended = yield* appendNote(input.sessionID, newFacts, ids).pipe(
        ...
        Effect.catchCause((cause) =>
          Effect.gen(function* () {
            yield* Effect.logInfo("auto-memory append failed", { cause: String(cause).slice(0, 200) })
            return false
          }),
        ),
      )
        if (!appended) return
        for (const id of ids) seen.add(id)
        yield* Effect.logInfo("auto-memory: appended", { messages: targets.length, chars: newFacts.length })
      } finally {
...
export const node = LayerNode.make({
  service: Service,
  layer,
  deps: [LLM.node, Provider.node, Config.node, Memory.node],
})
