...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const check = Effect.fn("Goal.check")(function* (input: CheckInput) {
      ...
      if (!model) {
        yield* Effect.logWarning("no small checker model available; stopping goal loop")
        return true
      }
      const result = yield* llm
        ...
        .pipe(
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              yield* Effect.logInfo("goal check failed", { cause: String(cause).slice(0, 200) })
      ...
      yield* Effect.logInfo("goal check", { met, raw: result.slice(0, 120) })
      return met
    })
...
export const node = LayerNode.make({
  service: Service,
  layer,
  deps: [LLM.node, Provider.node, Config.node],
})
