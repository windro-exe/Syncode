...
export const Event = {
  ...
  Deleted: EventV2.define({
    type: "memory.deleted",
    schema: { scope: Scope, sessionID: Schema.optional(SessionID), path: Schema.String },
  }),
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const tryDb = <A, E, R>(eff: Effect.Effect<A, E, R>) =>
      eff.pipe(
        Effect.mapError(
          (e) => new MemoryError({ message: `memory storage error: ${e instanceof Error ? e.message : String(e)}` }),
        ),
      )
    ...
    const create: Interface["create"] = Effect.fn("Memory.create")(function* (input) {
      ...
      yield* events.publish(Event.Created, entry)
      return entry
    })
    ...
    const updateContent = Effect.fn("Memory.updateContent")(function* (
      ...
      const entry = row2entry(row!)
      yield* events.publish(Event.Updated, entry)
      return entry
    })
    ...
    const remove: Interface["remove"] = Effect.fn("Memory.remove")(function* (input) {
      ...
      const exact = yield* fetchOne(input.scope, p, sessionID)
      if (exact) {
        yield* tryDb(db.delete(MemoryEntryTable).where(eq(MemoryEntryTable.id, exact.id)).run())
        yield* events.publish(Event.Deleted, { scope: input.scope, sessionID, path: p })
        return
      }
      ...
      for (const row of matched) {
        yield* tryDb(db.delete(MemoryEntryTable).where(eq(MemoryEntryTable.id, row.id)).run())
        yield* events.publish(Event.Deleted, { scope: input.scope, sessionID, path: row.path })
      }
    })
    ...
    const search: Interface["search"] = Effect.fn("Memory.search")(function* (input) {
      ...
      const rows = yield* tryDb(
        db.all<typeof MemoryEntryTable.$inferSelect & { bm25: number }>(sql`
    ...
    const forget: Interface["forget"] = Effect.fn("Memory.forget")(function* (input) {
      ...
      for (const v of victims) {
        yield* tryDb(db.delete(MemoryEntryTable).where(eq(MemoryEntryTable.id, v.id)).run()).pipe(
          Effect.orElseSucceed(() => undefined),
        )
      }
      for (const v of victims)
        yield* events.publish(Event.Deleted, { scope: v.scope, sessionID: v.session_id ?? undefined, path: v.path })
